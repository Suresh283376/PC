...
class OtherTransactionHelper {
  ...
  property get OtherTransactions(): IQueryBeanResult<Job> {
    ...
    other.orderBy(QuerySelectColumns.path({Job.SELECTEDVERSION_PROP.get(), PolicyPeriod.PERIODID_DYNPROP.get(PolicyPeriod)}))
    other.thenBy(QuerySelectColumns.path({Job.SELECTEDVERSION_PROP.get(), PolicyPeriod.MODELNUMBER_PROP.get()}))
    return other
  }
