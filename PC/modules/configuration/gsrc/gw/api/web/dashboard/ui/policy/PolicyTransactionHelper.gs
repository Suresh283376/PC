...
class PolicyTransactionHelper {
  ...
  static function retrievePolicyTransactions(accounts: Account[], completed: Boolean): Job[] {
    ...
    moveCancellationsToTopAndRenewalsToBottom(jobs)
    return jobs
  }
  ...
  static private function moveCancellationsToTopAndRenewalsToBottom(jobs: Job[]) {
    jobs.sort(Comparator.comparingInt(\job: Job -> priorityBySubtype.getOrDefault(job.Subtype, 0)))
  }
