...
class HOPDwellingValidation extends PCValidationBase {
   ...
   *   <li>The garage type is null or "No Garage" if the coverage part type is not dwelling</li>
   * </ul>
   */
  ...
  private function checkGarageValid() {
    var coveragePart = _hopDwelling.HOPCoveragePart.CoveragePartType
    if (coveragePart != CoveragePartType.TC_HOPDWELLING and
        _hopDwelling.GarageType != null and _hopDwelling.GarageType != GarageType.TC_NONE) {
      var reason = DisplayKey.get("Web.Policy.HOP.Validation.GarageNotAllowed", coveragePart.DisplayName)
      Result.addFieldError(_hopDwelling, "GarageType", ValidationLevel.TC_DEFAULT, reason, HOP_DWELLING_CONSTRUCTION_WIZARD_STEP)
    }
  }
