...
class RateRoutineEditScreenHelper {
  ...
  private function collectLoopSections(): ImmutableListMultimap<CalcStepDefinition, CalcStepDefinition> {
    ...
    _orderedSteps.stream().forEachOrdered(\step -> {
  ...
  function copySteps(rows: CalcStepDefinition[], currentSection: RateRoutineSection): RateRoutineSection {
    ...
    for (step in sortedRows) {
      ...
      makeSpaceAfter(lastPosition, isLoopBlock ? getLoopBlockSize(step) : 1)
      if (isLoopBlock) {
        final var offset = lastPosition + 1 - step.SortOrder
        getLoopBlock(step).stream().forEach(\blockStep -> {
  ...
  function removeRow(step: CalcStepDefinition, currentSection: RateRoutineSection = null): RateRoutineSection {
    ...
    if (stepStartsLoopBlock(step)) {
      LoopSections.get(step).stream().forEach(\inLoopStep -> _rateRoutine.removeFromSteps(inLoopStep))
      removeSpaceAt(step.SortOrder, LoopSections.get(step).last().SortOrder + 1)
    } else {
  ...
  private function moveLoopSectionDown(loopStep: CalcStepDefinition, offset: int) {
    getLoopBlock(loopStep).stream().forEach(\sectionStep -> { sectionStep.SortOrder += offset })
  }
   ...
   */
  private function removeSpaceAt(stepNum: int, upToStepNum: int = 0) {
    if (upToStepNum == 0) {
      upToStepNum = stepNum + 1
    }
    var rowsToMove = _rateRoutine.Steps.where(\row -> row.SortOrder > stepNum and row.SortOrder >= upToStepNum)
    var range = upToStepNum - stepNum
    rowsToMove.each(\row -> {
      row.SortOrder = row.SortOrder - range
    })
  }
