...
abstract class JobProcess implements gw.job.IJobProcess {
  ...
  private final var _ilElement: ILElement as ILElement = ILElement.of(
      ILElementIdentifier.of(ILElementType.TC_JOB,"AbstractJobProcess"),
      PCLoggingMarker.JOB_PROCESS
  )
  ...
  private function startJobWithStatus(status: PolicyPeriodStatus) {
    ...
    IntentionalLogger.logStart(ILElement)
...
//    JobProcessContextUpdater.updateContext(_branch)
  ...
  override function requestPostRatingQuote(jobWizardHelper: JobWizardHelper) {
    try {
      ...
      PCProfilerTag.JOB_REQUEST_QUOTE.execute(\-> quoteProcess.requestPostRatingQuote(jobWizardHelper))
    } finally {
      _isQuoting = false
      BeanCache.clear()
    }
