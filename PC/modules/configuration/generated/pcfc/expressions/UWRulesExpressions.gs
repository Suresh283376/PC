...
class UWRulesExpressions {
  ...
  public static class IteratorEntryExpressionsImpl extends SearchPanelExpressionsImpl {
    ...
    function action_47 () : void {
      UWRuleDetail.go(helper.getHead(helper.RuleQueryResults.getRuleVersion(rule)), helper.RuleQueryResults.getRuleVersion(rule))
    }
    
    // 'action' attribute on TextCell (id=Name_Cell) at UWRules.pcf: line 136, column 36
    function action_dest_48 () : pcf.api.Destination {
      return pcf.UWRuleDetail.createDestination(helper.getHead(helper.RuleQueryResults.getRuleVersion(rule)), helper.RuleQueryResults.getRuleVersion(rule))
    }
